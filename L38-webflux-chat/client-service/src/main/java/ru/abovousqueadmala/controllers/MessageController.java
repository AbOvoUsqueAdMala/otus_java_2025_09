...
public class MessageController {
    ...
    public void getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        if (isAggregateRoom(roomId)) {
            logger.warn("ignoring message for read-only room:{}", roomId);
            return;
        }
        ...
        template.convertAndSend(buildTopic(roomId), escapedMessage);
        template.convertAndSend(buildTopic(AGGREGATE_ROOM_ID), escapedMessage);
    }
    ...
    private String buildTopic(String roomId) {
        return String.format("%s%s", TOPIC_TEMPLATE, roomId);
    }
}
